...
await p.evaluate(() => {
    if (window.__agentPickerInstalled) {
        return;
    }
...
console.log("Picker ready – click the element and press Enter when done…");
const result = await p.evaluate((msg) => window.__agentPick(msg), message);
